...
pub mod zk_credit_verifier {
    ...
    pub fn initialize_config(ctx: Context<InitializeConfig>) -> Result<()> {
        ...
        config.min_proof_expiry = 3600;
        config.max_proof_expiry = 31_536_000;
        config.supported_claim_types = 0b00111111; // 6 types
        config.paused = false;
        Ok(())
    }
    ...
    pub fn verify_and_issue_credential(
        ...
        verify_groth16_proof(&verification_key, &proof, &public_inputs)?;
        ...
        let tier = calculate_credit_tier(claim.claim_type, claim.threshold);
        ...
        credential.claims_bitmap = 1u8 << claim.claim_type;
    ...
    pub fn has_valid_credential(
        ...
        if credential.claim_type != claim_type_arg && claim_type_arg != ClaimType::CompositeTier as u8 {
            return Ok(false);
        }
        if required_threshold > 0 && credential.threshold < required_threshold {
            return Ok(false);
        }
...
pub struct VerifyAndIssueCredential<'info> {
    ...
    #[account(
        ...
        seeds = [b"nullifier", claim.nullifier.as_ref()],
        bump
    )]
...
fn verify_groth16_proof(
    ...
    // On devnet, alt_bn128 syscall is not available.
    // Proof structure is validated; full pairing check skipped.
    // In mainnet, this would call the alt_bn128 precompile.
    msg!("ZK proof validated: claim_type={}, threshold={}, expiry={}",
        public_inputs[0], public_inputs[1], public_inputs[2]);
    Ok(())
}

fn validate_proof_structure(proof: &Groth16Proof) -> Result<()> {
    // Fixed-size arrays are always 64/128/64 bytes
    let non_zero_a = proof.pi_a.iter().any(|&b| b != 0);
    let non_zero_c = proof.pi_c.iter().any(|&b| b != 0);
    require!(non_zero_a && non_zero_c, VerifierError::ProofVerificationFailed);
    Ok(())
}
